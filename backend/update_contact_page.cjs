...
async function update() {
    ...
    try {
        const [pages] = await conn.execute('SELECT id FROM web_paginas_v4 WHERE slug = "contacto"');
        if (pages.length > 0) {
            ...
            await conn.execute('UPDATE web_paginas_v4 SET content = ? WHERE id = ?', [JSON.stringify(newContent), pageId]);
